...
We use "know" descriptively: the correct answer was structurally available in the model's top-k logprob distribution but not committed to. Empirically, this commitment gap is a stable, measurable pattern — retrieval succeeds (the correct token carries substantial probability mass) but commit fails (a competing token wins). It is statistical structure, not arbitrary noise. The practical consequence — which this paper establishes empirically and which does not depend on any particular theory of why the gap exists — is that a commitment gap can be closed by re-prompting the model under different conditions, causing resampling that sometimes lands on the correct alternative. A genuine knowledge gap, where the correct answer has negligible probability, cannot. Under the companion framework (Lund 2026b §9.1b) this pattern is interpreted as a "friction ceiling" arising from race-resolution under bounded resources; that interpretation is offered as context, not as a premise the empirical results below rely on.
...
Extracting the CR signal is free: it requires only `logprobs=True` in the API call — no additional model calls and no external verifier. This should not be confused with the pipeline being cost-free. The *signal* is a by-product of generation; the *pipeline built on it* still requires per-cell calibration on labelled examples (§3.3), and that labelled calibration set — not the signal — is the real cost of deployment. What the signal does buy is breadth: because CR measures a structural property of probabilistic token generation, the same extraction code works across the architectures, model families, and benchmarks we tested, without per-architecture signal engineering.
...
**The signal transfers across the architectures tested.** CR works not only on standard dense transformers (Qwen2.5, GPT-oss) but also on a mixture-of-experts model (Qwen3-235B) and on LiquidAI's Liquid Neural Network — a non-attention architecture based on continuous-time dynamical systems. The same extraction code, with no per-architecture signal engineering, produces significant lifts on all four models. We frame this as evidence that the signal generalises across the autoregressive LM architectures in our panel, not as a claim of full architectural universality: the MoE model is still transformer-based, and the single Liquid Neural Network result is one model on one benchmark with a low vanilla baseline (§4.4). Establishing universality would require a wider architecture panel, including stronger non-attention models; we treat it as an open empirical question.
...
The name "friction-guided" reflects the origin of this work in a companion theoretical framework (Lund 2026b, **Friction Theory**), which motivated the search for a logprob-derived signal in the first place. That framework supplies vocabulary and interpretation, and a reader interested in why the signal behaves as it does will find the account there. The present paper, however, is written to stand on its own: every empirical claim below is stated in standard terms — logprob distributions, calibration sets, accuracy lifts, abstention thresholds — and can be evaluated without accepting any claim from the companion paper. Where we use framework vocabulary ("friction", "commitment gap", "race-resolution"), it is signposted as interpretation and is not load-bearing for the results.
...
### 5.6 Transfer Across Architectures

A notable finding is that the pipeline works on LiquidAI's Liquid Neural Network — a non-attention architecture based on continuous-time dynamical systems rather than discrete attention. The CR signal, which counts competing token candidates, is computed from the output logprob distribution and so does not depend on the internal computation that produced it: any autoregressive model that exposes top-k logprobs admits the same extraction code.

We are deliberately cautious about how far to generalise this. The empirical claim is bounded: the signal extracts and the pipeline produces significant lifts on the four models in our panel — two dense transformers, one mixture-of-experts transformer, and one Liquid Neural Network. The MoE model is still transformer-based, and the Liquid Neural Network is a single model on a single benchmark with a low vanilla baseline (33.8%), where part of the lift may reflect a weak base prompt rather than architecture-general friction (§4.4). We therefore report this as *transfer across the architectures tested*, not as architectural universality. A stronger test would add non-attention models with higher baselines (e.g., state-space models such as Mamba or Jamba); we leave that to future work. The companion framework (Lund 2026b) offers a theoretical reason to expect broad transfer — it treats friction as a property of probabilistic computation rather than of any specific architecture — but the present paper neither needs nor tests that stronger claim.
...
- **Model drift**: Together.ai updated the Cogito checkpoint between our calibration and deployment runs, changing vanilla accuracy by roughly 16–19 pp. We mitigate this in our reported results by measuring lift as paired within-question rescue (improvement against the model's own vanilla answer in the same session), which is drift-robust. But this mitigation is partial, and the limitation is more serious than a single caveat sentence conveys. Three consequences are worth stating plainly. (i) *Cross-session reproducibility*: absolute accuracy numbers on cloud-served checkpoints are session-specific and will not reproduce exactly; only the paired within-question comparisons are stable. (ii) *Calibration transfer*: a calibration computed before a checkpoint update may no longer be optimal afterwards — strategy rankings and thresholds may need re-running. (iii) *Survivorship risk*: when a checkpoint update is suspected mid-experiment, paired rescue measured only on the post-update vanilla-wrong subset can be biased, because the post-update model fails a different set of questions than the pre-update model. We therefore recommend that calibration and deployment occur in the same session against a checkpoint whose identity is logged, and we report platform and approximate date for every cell. This is a property of commercial cloud-inference infrastructure that all LLM benchmarking must contend with; it is not specific to our method, but it does bound the reproducibility of any result reported against a cloud endpoint.
...
### 5.8 Optional: iterative adaptation as reflexive matching

*This section is an optional theoretical aside. It interprets the empirical result of §5.1 — that follow-up strategies applied to already-answered questions outperform standalone strategies — through the companion framework. None of the results in §4 or the recommendations in §5.1–§5.5 depend on it; a reader uninterested in the theory can skip to §6.*
...
The method is practical: it requires no model training and no external verifier models. It does require a labelled calibration set of 50–200 questions per cell — that set, not the signal, is the real deployment cost. The signal itself is free to extract, and the pipeline adds 1–4 inference calls per question. The audit harness used to produce these results — automated pre-flight checks, oracle-plateau verification, oracle-cheat detection — is released with the code so the pipeline can be re-run with the same safeguards.
...
- Liu, A., Wu, Z., Michael, J., Suhr, A., West, P., Koller, A., Swayamdipta, S., Smith, N. A., & Choi, Y. (2023). We're Afraid Language Models Aren't Modeling Ambiguity. *EMNLP 2023*.
